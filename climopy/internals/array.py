...
class _ArrayContext(object):
    ...
    def __init__(
        ...
        *datas : numpy.ndarray
        ...
        push_left, push_right : int or list of int, optional
            Axis or axes to move to the left or right sides.
            By default, if neither are provided, `push_right` is set to ``-1``.
        nflat_left, nflat_right : int, optional
            Number of dimensions to flatten on the left or right sides.
            By default, if only `push_left` is provided, `nflat_right` is set to
    ...
    def __enter__(self):
        ...
        for array in arrays:
            ...
            array = self._execute_axis_moves(array, moves)
            ...
            if shape != reshape:
                ...
                array = np.reshape(array, reshape, order='C')
    ...
    def __exit__(self, *args):  # noqa: U100
        ...
        for array, ishape, imoves in zip(arrays, shapes, moves):
            ...
            if array.shape != ishape:
                ...
                array = np.reshape(array, ishape, order='C')
            array = self._execute_axis_moves(array, imoves, reverse=True)
            self._arrays.append(array)

    @staticmethod
    def _execute_axis_moves(array, moves, reverse=False):
        """
