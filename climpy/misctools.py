...
def match(*args):
    ...
    slices = [slice(min_i, max_i + 1)
              for min_i, max_i in zip(min_locs, max_locs)]
    if any(v[slice_i].size != vs[0][slices[0]].size for v,
           slice_i in zip(vs, slices)):
        raise ValueError(
...
def intersection(x, segment1, segment2, xlog=False):
    ...
    if xlog:  # transform x coordinates optionally
        def transform(x): return np.log10(x)
        def itransform(x): return 10**x
    else:
        def transform(x): return x
        def itransform(x): return x
    ...
    # once again for the y-position; can also use segment1
