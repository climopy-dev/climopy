...
def eqlat(lon, lat, q, skip=10, sigma=None, fix=False):  # n=1001, skip=10):
    ...
    # Determing Q contour values for evaluating eqlat (no interpolation; too
    # slow)
    K = q.shape[-1]  # number of extra dims
    # e.g. // counts number of complete length-<skip> blocks after index 0,
    # then add 0 position
    N = (np.prod(shape[:2]) - 1) // skip + 1
    # want to center the list if possible; e.g. [0,1,2,3,4,5,6,7,8], skip=3,
    # will give [1,4,7]
    ...
    for k in range(K):
        ...
        for n in range(N):
            ...
            if np.isnan(band):  # happens if contours intersect at edge
                ...
                # high anomalies at low lat (below top graticule)
    ...
    for k in range(K):
        ...
        for n in range(N):
            ...
            if np.isnan(band):
                ...
                # anom = q[:,:,k] - q_bands[0,n,k]
                # should be identical, since positive/negative regions have
